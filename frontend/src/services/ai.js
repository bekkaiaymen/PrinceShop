...
class AIService {
  ...
  async searchProducts(query, products) {
    try {
      ...
      // Prompt محسّن مع أمثلة واضحة
      const prompt = `أنت محرك بحث للمنتجات. ابحث في القائمة عن منتجات تطابق الاستعلام.
...
- شاحن = CHARGEUR
- كابل = CABLE
- بلوتوث = BLUETOOTH, SANS FIL
- مكبر = BAFFLE, SPEAKER
- انكر = ANKER (علامة تجارية)
- هوكو = HOCO (علامة تجارية)

✅ أمثلة:
- "سماعات بلوتوث" → ابحث عن AIRPODS أو CASQUE مع BLUETOOTH
- "منتجات انكر" → ابحث عن ANKER في اسم المنتج
- "شاحن hoco" → ابحث عن CHARGEUR و HOCO معاً

⚠️ مهم: إذا ذُكرت علامة تجارية (ANKER, HOCO), ابحث عنها بالضبط في الاسم.

رد فقط بأرقام المنتجات المطابقة (مثال: 0,5,12) أو NONE`;
