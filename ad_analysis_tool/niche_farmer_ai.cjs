...
async function runNicheFarmer() {
    ...
    try {
        ...
        // --- AI INJECTION START ---
        const HOT_PRODUCTS_FILE = path.join(__dirname, 'hot_products.json');
        if (fs.existsSync(HOT_PRODUCTS_FILE)) {
            try {
                const hotProducts = JSON.parse(fs.readFileSync(HOT_PRODUCTS_FILE, 'utf8'));
                if (Array.isArray(hotProducts) && hotProducts.length > 0) {
                    console.log(`🔥 AI INJECTION: Found ${hotProducts.length} hot products from Telegram/Gemini.`);
                    console.log(`   Products: ${hotProducts.join(', ')}`);
                    
                    // Inject into ALL target niches
                    targetNiches.forEach(niche => {
                        // Prepend to ensure they are used first
                        niche.keywords = [...hotProducts, ...niche.keywords];
                        console.log(`   -> Injected into Niche '${niche.name}'`);
                    });
                }
            } catch (err) {
                console.error("❌ Failed to load hot_products.json:", err.message);
            }
        }
        // --- AI INJECTION END ---
...
async function warmUpFacebook(page, keywords) {
    // Select keywords for warming
    // STRATEGY: Heavily prioritize "Hot Products" (Verified via Telegram/AdLib) which are at the start of the list.
    
    // 1. Identify "Hot" pool (Top 6 keywords)
    // These are the ones we injected from hot_products.json
    const hotPool = keywords.slice(0, 6);
    
    // 2. Identify "Niche/Generic" pool (The rest)
    const nichePool = keywords.slice(6);
    
    // 3. Selection Logic:
    // We want to perform 3-4 searches per warmup cycle.
    // We take 3 from Hot Pool (if available) and 1 from Niche Pool.
    
    const selectedTerms = [];
    
    // Pick 3 random hard-hitters from Hot Pool
    if (hotPool.length > 0) {
        const shuffledHot = [...hotPool].sort(() => 0.5 - Math.random());
        selectedTerms.push(...shuffledHot.slice(0, 3));
    }
    
    // Pick 1 random exploration term from Niche Pool
    if (nichePool.length > 0) {
        const randomNiche = nichePool[Math.floor(Math.random() * nichePool.length)];
        selectedTerms.push(randomNiche);
    }
    
    // Deduplicate just in case
    const searchTerms = [...new Set(selectedTerms)];

    console.log("🔥 Warming up FB algorithm with Verified Products:", searchTerms);
    ...
    for (const term of searchTerms) {
        try {
            ...
            // Look for generic feed links (posts, videos, groups)
            // Updated selector to be more inclusive of search result types
            const resultLinks = await page.$$('div[role="feed"] a[role="link"], div[role="main"] a[role="link"]');
            ...
            for (const link of resultLinks) {
                try {
                    ...
                    // Check if it's a valid post/video/group link and NOT a marketplace or friend link
                    if (href && href.length > 25 && 
                        !href.includes('/friends/') && 
                        !href.includes('/marketplace/') && 
                        !href.includes('/watch/') &&
                        (href.includes('/posts/') || href.includes('/groups/') || href.includes('/videos/') || href.includes('profile.php') || href.includes('?id='))
                       ) {
...
async function collectAds(page) {
    ...
    // 1. Initial Navigation - AVOID ROOT URL ('/') to prevent White Page bug.
    // We use the "Most Recent" feed directly, which is lighter and less prone to rendering freezes.
    const targetUrl = 'https://www.facebook.com/?sk=h_chr';
    await page.goto(targetUrl, { waitUntil: 'domcontentloaded' });
    await delay(5000); 

    // --- BLANK PAGE DETECTION & RECOVERY ---
    try {
        const feedSelector = 'div[role="feed"], div[data-pagelet^="FeedUnit"], div[role="main"]';
        
        // Helper to check if content exists
        const checkFeed = async () => !!(await page.$(feedSelector));

        let hasFeed = await page.waitForSelector(feedSelector, { timeout: 8000 }).then(() => true).catch(() => false);
        
        if (!hasFeed) {
            console.log("⚠️  BLANK PAGE detected on Feed. Switching strategies...");
            
            // Strategy: If 'Most Recent' fails, go to 'Watch' (Video Feed)
            // The Video Feed is a completely different react component and often works when Home fails.
            // It contains plenty of Video Ads which are high value.
            console.log("   🔄 Switching to WATCH Feed (Video Ads) to bypass Home verification...");
            
            await page.goto('https://www.facebook.com/watch', { waitUntil: 'domcontentloaded' });
            await delay(5000);
            
            // If still blank, try one explicit reload on Watch
            if (!await checkFeed()) {
            ...
            if (await checkFeed()) {
                console.log("✅ Recovered! Using Video Feed for Ad Collection.");
            } else {
                console.error("❌ CRITICAL: Facebook UI is refusing to render content.");
            }
        } else {
            console.log("✅ Feed loaded successfully.");
        }
    } catch (e) { console.log("   (Blank page check skipped)", e.message); }
    // ----------------------------------
    ...
    console.log("    (Using HYBRID SCROLL: Spacebar + Mouse Wheel + JS to ensure feed trigger)");

    let noChangeCount = 0;
    let lastHeight = 0;

    // Focus the page body to ensure keyboard commands work
    await page.click('body').catch(() => {});

    while (true) {
        try {
            // METHOD 1: Spacebar (Often forces feed load more reliably than wheel)
            if (Math.random() > 0.4) {
                 await page.keyboard.press('Space');
                 await delay(500); 
            }

            // METHOD 2: Mouse Wheel (Simulate User Reading)
            for(let i=0; i<3; i++){
                await page.mouse.wheel({ deltaY: 200 }); 
                await delay(200);
            }
            
            // Wait for content load (API Response time)
            await delay(3000); 

            // METHOD 3: JS Scroll (Fallback if UI inputs fail)
            // Scroll to 80% of page height to trigger "Load More"
            await page.evaluate(() => {
                window.scrollTo(0, document.body.scrollHeight - 300);
            });

            // Occasional 'human' movement
            if (Math.random() > 0.7) {
            ...
            if (currentHeight === lastHeight) {
                noChangeCount++;
                if (noChangeCount > 4) { // Stuck for ~12-15 seconds
                    console.log("⚠️ Feed stuck. Force reloading content area...");
                    
                    // Maneuver: Scroll TO TOP to trigger 'Pull to Refresh' logic if present, then BACK DOWN
            ...
            // CRITICAL: Keep preventing White Page
             const pageContent = await page.content();
             if (pageContent.length < 5000) {
                 console.log("⚠️ Page seems blank/crashed (Content length low). Reloading...");
